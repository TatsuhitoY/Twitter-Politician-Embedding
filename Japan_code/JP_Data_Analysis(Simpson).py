...
for i in range(len(identity_list)):
    ...
    all_name_list.append(f'{identity_list[i][4]} {identity_list[i][3]} {identity_list[i][0]}')
...
filename = '/Users/yoshiharatatsuhito/Desktop/T.Yoshihara/計算社会科学/Twitter_congress/整形済みデータ/Simpson(JP).csv'
with open(filename, encoding='utf8', newline='') as f:
    csvreader = csv.reader(f)
    a_b_jt=[]
    for row in csvreader:
...
bar1 = tqdm(total = len(matrix), colour = 'green')
matrix_show = []
...
bar2 = tqdm(total = len(H.nodes()), colour = 'green')
c = []
for i in H.nodes():
  bar2.update(1)
  n = 0
...
if sign1 == 1:
    print("m(全ての辺の重みの合計): {}".format(m_))
# print("Nodes in the graph:", H.nodes())
# pos = nx.kamada_kawai_layout(H)
# plt.figure(figsize=(21,14))
# plt.title('Simpson index Threshold:{}'.format(simpson係数))
nx.write_gexf(H, f'/Users/yoshiharatatsuhito/Downloads/JP_Rep_n(Simpson={simpson係数}).gexf')
# nx.draw_networkx(H, pos, node_size = size, node_color=c, arrows=False, width= edge_width, font_family='Hiragino Sans')
# pickle.dump(H, open(f'/Users/yoshiharatatsuhito/Downloads/JP_G_simpson={simpson係数}', 'wb'))
# # plt.show()

...
#Dice係数のファイルから値を取り出す
filename = '/Users/yoshiharatatsuhito/Desktop/T.Yoshihara/計算社会科学/Twitter_congress/整形済みデータ/Dice(JP).csv'
with open(filename, encoding='utf8', newline='') as f:
    csvreader = csv.reader(f)
    a_b_jt=[]
    for row in csvreader:
...
for i in range(len(matrix)):
  if matrix[i][2] >= dice係数 / coefficient_adjust  and matrix[i][2] != 1:
...
plt.title('Dice index Threshold:{}'.format(dice係数))
nx.write_gexf(H, f'/Users/yoshiharatatsuhito/Downloads/JP_Rep(Dice={dice係数}).gexf')

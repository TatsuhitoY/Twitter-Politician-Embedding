...
for a in total_list:
    ...
    for b in total_list:
        ...
        if d > c:
            ...
            cal_num = (len(common))/((len(a)-1)+(len(b)-1)-(len(common)))
            if cal_num >= 0.9:
                print(f'{a[-1]} {b[-1]}')
            jaccard_m.append(cal_num)

e = datetime.datetime.now()
print(f'Computation Time：{e - s}')
f = open('/Users/yoshiharatatsuhito/Desktop/T.Yoshihara/計算社会科学/Twitter_congress/整形済みデータ/Jaccard(JP).csv', 'w')
data = jaccard_m
writer = csv.writer(f)

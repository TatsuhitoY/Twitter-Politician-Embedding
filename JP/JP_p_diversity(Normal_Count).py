...
print(f'\n自民 重複あり{len(jimin_list)}　重複なし{len(set(jimin_list))}　平均フォロー数{round(len(jimin_list)/len(set(jimin_list)), 3)}')
print(f'立憲 重複あり{len(rikken_list)}　重複なし{len(set(rikken_list))}　平均フォロー数{round(len(rikken_list)/len(set(rikken_list)), 3)}')
print(f'維新 重複あり{len(ishin_list)}　重複なし{len(set(ishin_list))}　平均フォロー数{round(len(ishin_list)/len(set(ishin_list)), 3)}')
print(f'公明 重複あり{len(koumei_list)}　重複なし{len(set(koumei_list))}　平均フォロー数{round(len(koumei_list)/len(set(koumei_list)), 3)}')
print(f'共産 重複あり{len(kyousan_list)}　重複なし{len(set(kyousan_list))}　平均フォロー数{round(len(kyousan_list)/len(set(kyousan_list)), 3)}')
print(f'れ新 重複あり{len(reshin_list)}　重複なし{len(set(reshin_list))}　平均フォロー数{round(len(reshin_list)/len(set(reshin_list)), 3)}')
print(f'国民 重複あり{len(kokumin_list)}　重複なし{len(set(kokumin_list))}　平均フォロー数{round(len(kokumin_list)/len(set(kokumin_list)), 3)}')
print(f'社民 重複あり{len(syamin_list)}　重複なし{len(set(syamin_list))}　平均フォロー数{round(len(syamin_list)/len(set(syamin_list)), 3)}')
print(f'その他 重複あり{len(other_list)}　重複なし{len(set(other_list))}　平均フォロー数{round(len(other_list)/len(set(other_list)), 3)}')
...
plt.suptitle(f'Minimum number of follow: {switch}, Normal_Count Similarity')
plt.show()

...
#New_cal係数のファイルから値を取り出す
filename = '/Users/yoshiharatatsuhito/Desktop/T.Yoshihara/計算社会科学/Twitter_congress/整形済みデータ/New_cal(US).csv'
with open(filename, encoding='utf8', newline='') as f:
    csvreader = csv.reader(f)
    a_b_jt=[]
    for row in csvreader:
...
for i in range(len(matrix)):
  if matrix[i][2] >= new_cal係数 / coefficient_adjust  and matrix[i][2] != 1:
...
plt.title('New_cal index Threshold:{}'.format(new_cal係数))
nx.write_gexf(H, f'/Users/yoshiharatatsuhito/Downloads/US_Rep(New_cal={new_cal係数}).gexf')
